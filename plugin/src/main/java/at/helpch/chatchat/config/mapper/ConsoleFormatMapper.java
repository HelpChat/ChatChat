...
public final class ConsoleFormatMapper implements TypeSerializer<ConsoleFormat> {
    ...
    @Override
    public ConsoleFormat deserialize(Type type, ConfigurationNode node) throws SerializationException {
        ...
        return new ConsoleFormat(key, parts);
    }

    @Override
    public void serialize(Type type, @Nullable ConsoleFormat format, ConfigurationNode target) throws SerializationException {
        if (format == null) {
            target.raw(null);
            return;
        }
